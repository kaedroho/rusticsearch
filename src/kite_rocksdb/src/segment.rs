...
impl<'a> Segment for RocksDBSegment<'a> {
    ...
    fn load_statistics(&self, stats: &mut Statistics) -> Result<(), String> {
        let kb = KeyBuilder::segment_stat(self.id, b"total_docs");
        if let Some(total_docs) = try!(self.reader.snapshot.get(kb.key())) {
            stats.total_docs += BigEndian::read_i64(&total_docs);
        }

        let kb = KeyBuilder::segment_stat(self.id, b"total_tokens");
        if let Some(total_tokens) = try!(self.reader.snapshot.get(kb.key())) {
            stats.total_tokens += BigEndian::read_i64(&total_tokens);
        }
