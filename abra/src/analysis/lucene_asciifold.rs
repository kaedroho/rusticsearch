...
pub fn fold_to_ascii(input: &str) -> String {
    ...
    for c in input.chars() {
        // Skip if character is already ASCII
/* TOOD: Check if this is worth it
        if c.is_ascii() {
            output.push(c);
            continue;
        }
...
mod tests {
    ...
    #[test]
    fn test_simple() {
        let string = fold_to_ascii("Ĥéllø");
    ...
    fn test_hiragana_not_changed() {
        let string = fold_to_ascii("こんにちは");

        assert_eq!(string, "こんにちは".to_string());
    }

    #[test]
    fn test_normalise_width_variants() {
        let string = fold_to_ascii("Ａ");

        assert_eq!(string, "A".to_string());
    }

    #[test]
    fn test_normalise_superscript() {
        let string = fold_to_ascii("i⁹");

        assert_eq!(string, "i9".to_string());
    }

    #[test]
    fn test_grapheme_clusters_keep() {
        // We want to keep certian grapheme clusters, such as this one (Devanagari kshi)
        let string = fold_to_ascii("क्षि");

        assert_eq!(string, "क्षि".to_string());
    }
}
